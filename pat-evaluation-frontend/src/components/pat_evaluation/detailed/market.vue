  ...
  <div class="market-detail">
    <title-canvas title="相关领域专利申请趋势">
      <div class="chart-wrap">
        <e-chart :options="applyTrends" :auto-resize="true" />
      </div>
    </title-canvas>

    <title-canvas title="相关领域市场交易趋势">
      <div class="chart-wrap">
        <e-chart :options="tradeTrends" :auto-resize="true" />
      </div>
    </title-canvas>
    ...
    <title-canvas title="相似专利及交易价格">
      <el-table :data="similarPatents" style="width:100%" :header-cell-style="tableHeaderStyle">
        ...
        <el-table-column prop="value" label="交易价格（万元）" width="160" align="right">
          <template slot-scope="scope">
            <span class="text-mono">{{ scope.row.value }}</span>
          </template>
        </el-table-column>
      </el-table>
    </title-canvas>
...
export default {
  data() {
    let applyTrends = _.cloneDeep(lineChartTemplate);
    applyTrends.xAxis.name = "年份";
    applyTrends.yAxis.name = "申请数量";
    let tradeTrends = _.cloneDeep(barChartTemplate);
    tradeTrends.xAxis.name = "年份";
    tradeTrends.yAxis.name = "交易金额(万元)";
    return {
      applyTrends,
      tradeTrends,
      similarPatents: [],
  ...
  methods: {
    reqSimlarPats(start, size) {
      axios.get(SIMILAR_PATENT_URL + "?patid=" + this.$store.state.patentId + "&start=" + start + "&size=" + size)
        .then((response) => {
  ...
  mounted() {
    ...
    if (patentId !== null) {
      axios.get(APPLY_TRENDING_URL + "?patid=" + patentId)
        .then((response) => {
          let data = response.data.apply_per_year.buckets;
          let years = [], counts = [];
          for (const i in data) {
            years.push(new Date(data[i].key).getFullYear());
            counts.push(data[i].doc_count);
          }
          this.applyTrends.xAxis.data = years;
          this.applyTrends.series[0].data = counts;
        }).catch((e) => console.log(e));

      this.reqSimlarPats(0, 5);

      axios.get(TRADE_TRENDING_URL + "?patid=" + patentId)
        .then((response) => {
          let data = response.data.apply_per_year.buckets;
          let years = [], amounts = [];
          for (const i in data) {
            years.push(new Date(data[i].key).getFullYear());
            amounts.push(data[i].trade_per_year.value);
          }
          this.tradeTrends.xAxis.data = years;
          this.tradeTrends.series[0].data = amounts;
        }).catch((e) => console.log(e));

      axios.get(ESTIMATE_PRICE_URL + "?patid=" + patentId)
        .then((response) => {
          let basePrice = response.data.price;
          this.patValue = [
            sprintf('%.2f', basePrice - basePrice * 0.35),
            sprintf('%.2f', basePrice + basePrice * 0.35)
          ];
        }).catch((e) => console.log(e));
    }
...
.price-currency { font-size: 18px; font-weight: 600; color: var(--color-text-secondary); }
.price-value { font-size: 32px; font-weight: 700; color: var(--color-primary); font-family: var(--font-mono); }
.price-sep { font-size: 20px; color: var(--color-text-tertiary); margin: 0 4px; }
.price-unit { font-size: 15px; font-weight: 500; color: var(--color-text-secondary); margin-left: 4px; }
</style>
