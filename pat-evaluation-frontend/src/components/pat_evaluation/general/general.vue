<template>
  <div class="general-overview">
    ...
    <div class="radar-card card">
      ...
      <p class="radar-note text-tertiary text-right">备注：各项评估项的最高取值均为 100</p>
    </div>

    <!-- 评估对话框 -->
    <el-dialog title="评估确认" :visible.sync="dialogVisible" width="420px" center>
      <p style="text-align:center; color: var(--color-text-secondary);">
        专利《{{ this.$store.state.patentTitle }}》尚未进行评估，是否立即评估？
      </p>
      <span slot="footer">
...
function buildRadarOption(values) {
  return {
    ...
    radar: {
      ...
      splitArea: { areaStyle: { color: ['#f0f5ff', '#f8faff', '#f0f5ff', '#f8faff'] } },
      splitLine: { lineStyle: { color: '#d6e4f0' } },
    ...
    series: [{
      ...
      symbol: 'circle',
      symbolSize: 8,
      itemStyle: { color: '#2b7de9', borderColor: '#fff', borderWidth: 2 },
      lineStyle: { color: '#2b7de9', width: 2.5 },
      areaStyle: {
        color: {
          type: 'radial', x: 0.5, y: 0.5, r: 0.5,
          colorStops: [
            { offset: 0, color: 'rgba(43,125,233,0.35)' },
            { offset: 1, color: 'rgba(43,125,233,0.08)' }
          ]
        }
      },
      data: [{
        ...
        name: '评估情况',
        label: {
          show: true, color: '#1a3a5c', fontWeight: 700, fontSize: 14,
...
export default {
  data() {
    return {
      patEvalue: [0, 0],
      chartData: buildRadarOption([0, 0, 0, 0]),
      dialogVisible: false
    };
  },
  mounted() {
  ...
  methods: {
    getData() {
      ...
      axios.get(START_URL + "?patid=" + patentId)
        .then((response) => {
          if (response.data['valid'] === false) { this.dialogVisible = true; return; }
          this.showEvaluation(response.data);
        })
        .catch((error) => { console.log(error); this.dialogVisible = true; });
    },
    startEvaluation() {
      let patentId = this.$store.state.patentId;
      axios.get(START_URL + "?patid=" + patentId + "&reload=1")
        .then((response) => { this.showEvaluation(response.data); this.dialogVisible = false; })
    ...
    showEvaluation(data) {
      ...
      this.patEvalue = [
        sprintf("%.2f", data.price[0]),
        sprintf("%.2f", data.price[1])
      ];
    },
...
<style scoped>
.general-overview { padding-bottom: var(--spacing-2xl); }
.price-card { text-align: center; padding: var(--spacing-xl); }
.price-display {
  display: flex; align-items: baseline; justify-content: center;
  gap: 6px; margin: var(--spacing-md) 0;
}
.price-currency { font-size: 20px; font-weight: 600; color: var(--color-text-secondary); }
.price-value {
  font-size: 36px; font-weight: 700; color: var(--color-primary);
  font-family: var(--font-mono); letter-spacing: -0.02em;
}
.price-sep { font-size: 24px; color: var(--color-text-tertiary); margin: 0 4px; }
.price-unit { font-size: 16px; font-weight: 500; color: var(--color-text-secondary); margin-left: 4px; }
.price-note { font-size: 13px; color: var(--color-text-tertiary); }
.radar-chart-wrap { display: flex; justify-content: center; padding: var(--spacing-lg) 0; }
.radar-note { font-size: 12px; margin-top: var(--spacing-sm); }
</style>
